...
import { ArrowLeft, ChevronLeft, CircleUserRound, Eye, EyeOff, Heart, Leaf, LockKeyhole, LogOut, MapPin, PackageCheck, Pencil, Plus, Settings, ShieldCheck, ShoppingBag, Truck, UserRound } from "lucide-react";
import "./account.css";
        ...
        <div className="auth-panel">
          <div className="auth-card">
            ...
            <form onSubmit={submitAuth}>
              ...
              <button className="auth-submit" type="submit" disabled={otpLoading}>{otpLoading ? "অপেক্ষা করুন..." : mode === "login" ? (otpSent ? "OTP যাচাই করে লগইন" : "OTP পাঠান") : "অ্যাকাউন্ট তৈরি করুন"}<ArrowLeft /></button>
            </form>
  ...
  return (
    <main className="account-page dashboard-page">
      ...
      <div className="account-container dashboard-grid">
        <aside className="account-sidebar"><nav><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}><CircleUserRound /> ওভারভিউ</button><button className={tab === "orders" ? "active" : ""} onClick={() => setTab("orders")}><ShoppingBag /> আমার অর্ডার <i>2</i></button><button className={tab === "addresses" ? "active" : ""} onClick={() => setTab("addresses")}><MapPin /> সংরক্ষিত ঠিকানা</button><button className={tab === "wishlist" ? "active" : ""} onClick={() => setTab("wishlist")}><Heart /> পছন্দের তালিকা <i>2</i></button><button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><Settings /> প্রোফাইল সেটিংস</button></nav><div><ShieldCheck /><p><strong>সাহায্য প্রয়োজন?</strong><small>সকাল ৯টা–রাত ১০টা</small></p><a href="tel:+8801886494257">কল করুন</a></div></aside>
        <section className="dashboard-content">
          {tab === "overview" && <><div className="overview-banner"><div><p>শুভ কেনাকাটা!</p><h2>আপনার সবকিছু<br />এক জায়গায়।</h2><Link href="/">নতুন পণ্য দেখুন <ArrowLeft /></Link></div><span><ShoppingBag /></span></div><div className="account-stats"><button onClick={() => setTab("orders")}><span><PackageCheck /></span><p><strong>২</strong><small>মোট অর্ডার</small></p></button><button><span><Truck /></span><p><strong>১</strong><small>ডেলিভারিতে</small></p></button><button onClick={() => setTab("wishlist")}><span><Heart /></span><p><strong>২</strong><small>পছন্দের পণ্য</small></p></button></div><RecentOrders /></>}
          {tab === "orders" && <div className="account-section"><div className="section-title"><div><p>আপনার কেনাকাটা</p><h2>আমার অর্ডার</h2></div><select><option>সব অর্ডার</option><option>চলমান</option><option>সম্পন্ন</option></select></div><RecentOrders all /></div>}
          {tab === "addresses" && <div className="account-section"><div className="section-title"><div><p>দ্রুত চেকআউট</p><h2>সংরক্ষিত ঠিকানা</h2></div><button onClick={() => setAddressOpen(!addressOpen)}><Plus /> নতুন ঠিকানা</button></div>{addressOpen && <form className="address-form" onSubmit={(event) => { event.preventDefault(); setAddressOpen(false); }}><input required placeholder="ঠিকানার নাম — যেমন: বাসা" /><textarea required placeholder="সম্পূর্ণ ঠিকানা" rows={3} /><div><input required placeholder="জেলা" /><input required placeholder="থানা / উপজেলা" /></div><button>ঠিকানা সংরক্ষণ করুন</button></form>}<div className="address-list"><article><span><MapPin /></span><div><p><strong>বাসা</strong><i>ডিফল্ট</i></p><address>বারিক ভিলা, ১১/১ ফোল্ডার স্ট্রিট,<br />ওয়ারী, ঢাকা–১২০৩</address><small>{user.phone}</small></div><button><Pencil /></button></article></div></div>}
          {tab === "wishlist" && <div className="account-section"><div className="section-title"><div><p>পরে কিনবেন</p><h2>পছন্দের তালিকা</h2></div></div><div className="wishlist-placeholder"><Heart /><h3>আপনার পছন্দগুলো এখানে থাকবে</h3><p>যে পণ্যগুলো ভালো লাগে, হার্ট আইকনে চাপ দিয়ে সংরক্ষণ করুন।</p><Link href="/">পণ্য দেখুন <ArrowLeft /></Link></div></div>}
          {tab === "profile" && <div className="account-section"><div className="section-title"><div><p>ব্যক্তিগত তথ্য</p><h2>প্রোফাইল সেটিংস</h2></div><button onClick={() => setEditingProfile(!editingProfile)}><Pencil /> {editingProfile ? "বাতিল" : "সম্পাদনা"}</button></div><form className="profile-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); saveUser({ name: String(data.get("name")), email: String(data.get("email") || ""), phone: String(data.get("phone")) }); setEditingProfile(false); }}><label><span>পুরো নাম</span><input name="name" defaultValue={user.name} disabled={!editingProfile} required /></label><label><span>ইমেইল <small>ঐচ্ছিক</small></span><input name="email" type="email" defaultValue={user.email} disabled={!editingProfile} placeholder="যোগ করা হয়নি" /></label><label><span>মোবাইল নম্বর</span><input name="phone" defaultValue={user.phone} disabled={!editingProfile} required /></label>{editingProfile && <button>পরিবর্তন সংরক্ষণ করুন</button>}</form></div>}
        </section>
      </div>
    </main>
  );
...
function RecentOrders({ all = false }: { all?: boolean }) {
  const orders = [{ id: "TM-2847163", date: "১৬ আগস্ট ২০২৬", status: "ডেলিভারিতে", total: "৳১,৪৩৫", icon: <Truck /> }, { id: "TM-2710924", date: "২৮ জুলাই ২০২৬", status: "ডেলিভারি সম্পন্ন", total: "৳১,৭৯০", icon: <PackageCheck /> }];
  return <div className="recent-orders"><header><div><p>সাম্প্রতিক</p><h2>{all ? "সব অর্ডার" : "সাম্প্রতিক অর্ডার"}</h2></div>{!all && <button>সব দেখুন <ArrowLeft /></button>}</header>{orders.map((order) => <article key={order.id}><span>{order.icon}</span><div><strong>#{order.id}</strong><small>{order.date}</small></div><p><i />{order.status}</p><strong>{order.total}</strong><button>বিস্তারিত <ChevronLeft /></button></article>)}</div>;
}
